(function () {
    ...
    function sectionSlider() {
        ...
        for (let i = 5; i < sectionSlider.length; i++) {
    ...
    const Calendar = (function () {
        ...
        // 從 Date 對象中獲取年、月和日
        const year = today.getFullYear(); // 年份（四位數，如：2024）
        const month = today.getMonth() + 1; // 月份（從 0 開始，所以需要加 1）
        const day = today.getDate();


        const startYear = 2024
        const startMonth = month
        const monthRange = 12 - month +1
              ...
              ${allYearMonths.map((month, i) => {
                  ...
                  <div class="swiper-slide">
                    <div class="capitalize text-28 fw-500 text-center mb-10">
                       ${startMonth + i > 12 ? startYear + 1 : startYear} ${month.monthName}
                    </div>
    ...
    function calendarInteraction() {
        ...
        target.forEach(elTarget => {
            ...
            gridCells.forEach((el, i) => {
                el.addEventListener('click', () => {
                    const selectedDate = `${el.getAttribute('data-month')} / ${el.querySelector('.js-date').innerHTML}`;

                    // 賦值 firstDate 和 lastDate 為點擊的日期
                    firstDate.innerHTML = selectedDate;
                    lastDate.innerHTML = selectedDate;
                    ...
                    // 移除所有日期的 -is-in-path 類
                    const inPathCells = elTarget.querySelectorAll('.-is-in-path');
                    inPathCells.forEach(cell => {
                        cell.classList.remove('-is-in-path');
                    });
                });
            });
        });
    ...
    function calendarInteraction2() {
        ...
        target.forEach(elTarget => {
            ...
            gridCells.forEach((el, i) => {
                el.addEventListener('click', () => {
                    const selectedDate = `2024-${el.getAttribute('data-month')}-${el.querySelector('.js-date').innerHTML}`;
